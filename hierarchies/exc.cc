...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        // is this dirty?
        if(fill_level==MSHR.entry[mshr_index].fill_level){
            if (block[set][way].dirty || (block[set][way].valid && cache_type>IS_L1I)) {
            ...
            /*else if(block[set][way].valid && (MSHR.entry[mshr_index].fill_l1d || MSHR.entry[mshr_index].is_data)) {
	  ...
	  if (cache_type == IS_L1I)
		...
		cpu = 0;
            ...
            if(fill_level==MSHR.entry[mshr_index].fill_level){
                ...
                block[set][way].dirty=MSHR.entry[mshr_index].is_dirty;
            }
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        /* 
        if(way<0 && cache_type!=IS_L1D && WQ.entry[index].ip==0 && WQ.entry[index].type==WRITEBACK){
            if (lower_level) { 
        ...
        */
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            if(WQ.entry[index].ip!=-3){
                // mark dirty
                if(cache_type==IS_L1D)
                    block[set][way].dirty=1;
                else
                    block[set][way].dirty = WQ.entry[index].is_dirty;
            }
            if (cache_type == IS_ITLB)
		...
		if (RQ.entry[index].type == LOAD) {
			...
			cpu = 0;
                ...
                if(fill_level<=FILL_LLC){
                    RQ.entry[index].is_dirty=block[set][way].dirty;
                    block[set][way].valid=0;
                    block[set][way].dirty=false;
                }
