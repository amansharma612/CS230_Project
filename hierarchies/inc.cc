    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
	  ...
	  if (cache_type == IS_L1I)
		...
		cpu = 0;
            ...
            if(block[set][way].valid && (MSHR.entry[mshr_index].fill_l1d || MSHR.entry[mshr_index].is_data) && fill_level!=FILL_L1){
                PACKET rem_pack;
                rem_pack.fill_level = -2;
                rem_pack.address=block[set][way].address;
                rem_pack.cpu = fill_cpu;
                //cout<<"hi"<<endl;
                //if(upper_level_dcache[fill_cpu]){cout<<"Present"<<endl;}
                //cout<<fill_cpu<<endl;
                //cout<<fill_level<<endl;
                upper_level_dcache[fill_cpu]->add_pq(&rem_pack);
                //cout<<"there"<<endl;
            }
...
void CACHE::handle_prefetch()
{
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
      ...
      /*if (prefetch_cpu == NUM_CPUS)
        return;
        if(PQ.entry[PQ.head].fill_level==-2){
            uint32_t set=get_set(PQ.entry[PQ.head].address),way;
            way=get_way(PQ.entry[PQ.head].address,set);
            if(way!=NUM_WAY){
                block[set][way].valid=0;
                if(fill_level!=IS_L1D){
                    PACKET temp;
                    temp.cpu=prefetch_cpu;
                    temp.address=PQ.entry[PQ.head].address;
                    temp.address=-2;
                    upper_level_dcache[prefetch_cpu]->add_pq(&temp);
                }
            }
            //cout<<"Hello"<<endl;
    ...
    if(packet->fill_level==-2){
        ...
        if(way!=NUM_WAY){
            ...
            if(fill_level!=FILL_L1){
                upper_level_dcache[packet->cpu]->add_pq(packet);
            }
        }
